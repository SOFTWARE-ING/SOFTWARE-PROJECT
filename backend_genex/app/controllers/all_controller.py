...
# ++++CREATE FUNCTION+++++
# +++++++++++++++++++++++++
def create_item(*, db: Session, model: Type[DeclarativeMeta], schema: BaseModel, use_uuid: bool = True):
    """
    Create a new SQLAlchemy object from a Pydantic schema.
    Auto-generates UUID if 'use_uuid' is True and model has 'id' column.
    """
    data = schema.model_dump(exclude_unset=True)

    if use_uuid and hasattr(model, "id") and "id" in model.__table__.columns:
